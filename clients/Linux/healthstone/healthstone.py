	...
	try:
		...
		if "checklusers" in cfg:
			try:
				...
				a, b, c = sys.exc_info()
				output += "--> [CheckLUsers] Error while fetching user listing: " + str(b) + "\n"
		if "checkprocesses" in cfg:
			try:
				...
				a, b, c = sys.exc_info()
				output += "--> [CheckProcesses] Error while fetching process information: " + str(b) + "\n"
		if "checkdiskspace" in cfg:
			try:
				...
				a, b, c = sys.exc_info()
				output += "--> [CheckDiskSpace] Error while fetching free disk space: " + str(b) + "\n"
